...
typedef struct {
    ...
    int        n_used;       /* slots currently allocated     */

    SlotMeta*  meta;         /* one entry per slot            */
    ...
    int        n_epochs;        /* epochs completed           */
    int        n_compactions;   /* compactions performed      */

    int        n_hot_slots;     /* slots in HOT zone          */
    int        n_warm_slots;
    int        n_cold_slots;
} HotColdSlab;
