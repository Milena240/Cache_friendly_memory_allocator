...
/* =========================================================
 * array_profiler.c  —  Implementation
 * ========================================================= */
...
/* ── Report ──────────────────────────────────────────────
 *
 * Sorts indices by access count (descending) so the
 * hottest elements appear at the top.
 * Prints a bar chart and hot/cold label for each.
 */
...
void print_access_report(ArrayProfiler* p,
    ...
    /* count how many elements were accessed at all */
    int n_accessed = 0;
    ...
    /* threshold: hot if > 1% of total accesses */
...
/* ── Reordering ──────────────────────────────────────────
 *
 * Builds a new array where hot elements come first.
 * Hot elements are sorted by access count (hottest first)
 * so the very hottest are in cache line 0.
 */
ReorderedArray* reorder_hot_first(ArrayProfiler* p,
    ...
    /* sort all indices by access count descending */
    IndexCount* sorted = (IndexCount*)malloc(
                             sizeof(IndexCount) * (size_t)p->size);
    for (int i = 0; i < p->size; i++) {
        sorted[i].index = i;
    ...
    /* build reordered array and mapping tables */
