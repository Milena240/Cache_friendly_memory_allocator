...
/* Hot and cold arrays are allocated separately so that:
 *   - hot[]  is a tight contiguous block  (cache-friendly iteration)
 *   - cold[] is a separate block          (only touched when needed)
 * hot[i].cold = &cold[i]  links them together. */
ObjectSplit* build_split_hot(int n) {
    ObjectSplit* hot = (ObjectSplit*)malloc(sizeof(ObjectSplit) * (size_t)n);
    for (int i = 0; i < n; i++) {
        hot[i].counter = i;
        hot[i].flags   = i % 256;
        hot[i].cold    = NULL;   /* filled in by build_split_cold */
    }
    return hot;
}
...
/* The hot work is always:
 *     counter++  and  flags ^= 0x01
 * The cold work (done every cold_every-th element) is:
 *     sum += weight
 * cold_every == 0 means never do cold work. */
...
long workload_unsplit(ObjectUnsplit* arr, int n, int cold_every) {
    ...
    return sum + arr[0].counter;   /* force use of arr */
}

long workload_split(ObjectSplit* arr, int n, int cold_every) {
    long sum = 0;
    for (int i = 0; i < n; i++) {
        /* hot work — touches only the 8-byte hot struct */
        arr[i].counter++;
        arr[i].flags ^= 0x01;
        /* cold work — follows pointer to cold block */
        if (cold_every > 0 && i % cold_every == 0)
